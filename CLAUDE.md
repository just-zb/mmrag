...
| # | Phase | Scope | Status |
|---|---|---|---|
| **P1a** | Image extraction (utility) | `DocxImageExtractor` walks `XWPFDocument` paragraphs, captures images with section heading + adjacent paragraph context. Pure utility, no DB / MinIO / ES side effects. | ✅ Done — `utils/DocxImageExtractor.java` + tests |
| **P1b** | Image persistence | MinIO bucket `images/{fileMd5}/{n}.{ext}` upload; new MySQL table `image_chunk`; per-tenant permission fields wired through. | ✅ Scaffolded — `entity/ImageChunk.java` + `repository/ImageChunkRepository.java` + `service/ImageStorageService.java` |
| **P1c** | ES schema extension | `vector` dim 2048 → 1152 (SigLIP-SO400M); add `content_type` (TEXT / IMAGE_UNIFIED / IMAGE_DESCRIPTION), `imageUri`, `caption`, `pageNum`. One-time reindex. | ✅ Scaffolded — `es-mappings/knowledge_base.json` + `entity/EsDocument.java` (three factory methods) |
| **P1d** | ParseService integration | Wire `DocxImageExtractor` + `ImageCaptionService` + `ImageStorageService` into the existing `.docx` ingestion path; emit image rows alongside text rows. PDF / `.pptx` / `.xlsx` paths remain Tika text-only and out of thesis scope. | ⏳ Open — final wiring not yet applied to `ParseService.parseAndSave()` |
| **P2** | Embedding adapter + image embedder | `EmbeddingClient` is now an interface with three adapters (`SiglipAdapter`, `CaptionBasedAdapter`, `DashScopeTextAdapter`); strategy switch in `application.yml`. | ✅ Scaffolded — `client/EmbeddingClient.java` + `client/embedding/*.java` |
| **P3** | Retrieval-strategy switch | `RetrievalStrategy` interface + four implementations (`BM25_ONLY`, `DENSE_ONLY`, `WEIGHTED_HYBRID`, `HYBRID_PLUS_RERANK`); Cohere Rerank v3 client. Per-request `mode`, `alpha`, `topK`, `rerankPoolSize`. | ✅ Scaffolded — `service/retrieval/*.java` + `client/CohereRerankerClient.java`. Final dispatch from the chat path is open. |
| **P4** | Evaluation harness | `RagasTracer` Spring interceptor on the chat path emitting one JSON line per query: `{ts, mode, params, query, retrieved[], answer, modality_breakdown}`. Standalone Python evaluator computing the four Ragas metrics over the trace files. | ✅ Scaffolded — `interceptor/RagasTracerInterceptor.java`. Wire-in at chat-handler exit is open. |
